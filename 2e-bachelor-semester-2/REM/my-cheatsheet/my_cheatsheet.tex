...
\secblock{H2 Electrostatics}
    ...
    or: \(W = \frac{\epsilon_0}{2}\int E^2d\uptau\)
...
Analog Biot-Savart:
\(\bm{E}(\bm{r})=-\frac{1}{4\pi}\frac{\partial}{\partial t}\int\frac{\bm B\times\hat{\brcurs}}{\brcurs^2}d\bm \uptau'\)
...
rate of work done:
\(\frac{dW}{dt} = \int_{V} (\bm J \cdot \bm E) d\uptau\)
...
\(\bm p = \frac{1}{c^2} \int \bm S d\uptau = \mu_0 \epsilon_0 \int \bm S d\uptau  \)
...
Retarded time:
\(t_r = t - \frac{\rcurs}{c}\) \(\implies \partial_{t_r} = \partial_t\) 
...
\(\bm A(\bm r, t) = \frac{\mu_0}{4\pi} \int \frac{\rho(\bm r', t_r) \bm v (t_r)}{\rcurs} d\uptau'\)
...
\(\brcurs = \bm r - \bm w (t_r)\) and \(\bm v = \dot{\bm w (t_r)}\)
...
Fields moving p-charge non-rel lim: 
...
\(\hat{\brcurs} = \frac{\bm r - \bm v t_r}{\rcurs} = \frac{(\bm r - \bm v t) + (t - t_r) \bm v}{\rcurs} = \frac{\bm R}{\rcurs} + \frac{\bm v}{c}\)
...
\(=  - \frac{\mu_0 p_0 \omega^2}{4 \pi} (\frac{\sin\theta}{r})\cos[\omega(t - \frac{r}{c})]\hat{\bm \theta}\)

\(\bm B(r, \theta , t) = \bm \nabla \times \bm A\)

\(= - \frac{\mu_0 p_0 \omega^2}{4 \pi} (\frac{\sin\theta}{r})\cos[\omega(t - \frac{r}{c})]\hat{\bm \phi}\)
...
Proper force:
\(\bm K = \frac{dt}{d\uptau} \frac{d\bm p}{dt} = \gamma_u \bm F\) ; \(K^0 = \frac{dp^0}{d\uptau} = \frac{1}{c} \frac{dE}{d\uptau}\)
...
Notation: \(\boxed{\partial_\mu = \frac{\partial}{\partial x^\mu}}\) and \(\boxed{\partial^\mu = \frac{\partial}{\partial x_\mu}}\)
