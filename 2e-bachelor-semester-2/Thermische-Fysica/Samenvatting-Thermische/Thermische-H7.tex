...
\begin{definition}[Pertuum mobile van de derde soort]   
    Een \textbf{pertuum mobile van de derde soort} is een hypothetisch apparaat dat in staat is om arbeid te verrichten zonder enige energiebron, door gebruik te maken van dissipatieve effecten. 
    Doordat dit niet in strijd is met de eerste twee wetten van de thermodynamican is er een derde wet nodig.
\end{definition}

\begin{definition}[Exterene Thermische irreversibiliteit]
...
Herinner u een algemeen systeem met veralgemeeende kracht \(Y\) en veralgemeende verplaatsing \(X\):
\[\dbar Q = dU -YdX\]
met \(U(T_e,X)\) een functie van de empirische temperatuur \(T_e\) en de veralgemeende verplaatsing \(X\).
\[
d U = \left(\frac{\partial U}{\partial T_e}_X\right) dT_e + \left(\frac{\partial U}{\partial X}_{T_e}\right) dX 
\]
\[
...
Eeen oplossing voor \(\frac{dT_e}{dX}\) is:
...
\[\dbar Q_H = \lambda_H(\sigma_R) d\sigma_R\] en \[\dbar Q_R = \lambda_R(\sigma_H) d\sigma_H\]
...
Zodat de coördinaten \(X_H'\) en \(X_R'\) kunnen worden geëlimineeerd uit de vergelijking van het samengestelde systeem, en we kunnen schrijven:
...
Veronderstel dat een reverisbel proces een warmte-overdracht van \(\dbar Q_c\), tussen het warmtereservoir en het gecombineerd systeem scheidt met een warmteoverdracht \(\dbar Q\)
...
met \(f(\sigma)d\sigma\) een totale differentiaal, en \(\frac{1}{\phi(T_e)}\) de intragerende factor.
\qed
\end{derivation}
...
Veronderstel dat een reversibele isotherme hoeveelheid warmte \(\dbar Q\) wordt uitgewisseld tussen ons systeeem en een warmtereservoir op \(T_e\). 
...
Twee temperaturen oop de Kelvinschaal verhouden zich zooals de warmteoverdracht tussen twee RA-oppervlakken bij die temperaturen.
...
\begin{figure}[h]
    ...
    \caption{\tiny \copyright{(The Engineering ToolBox, 2008). Carnot Cycle Efficiency. [online] Available at: https://www.engineeringtoolbox.com/carnot-efficiency-d\_1047.html [Accessed 14 May 2026].}}
\end{figure}
\begin{derivation}
