...
ax.scatter(verts[:,0], verts[:,1], verts[:,2], color='red', s=80, label='3-fold vertices (4)')
for i,v in enumerate(verts):
    ax.text(v[0]*1.05, v[1]*1.05, v[2]*1.05, f'v{i+1}', color='red')

# Plot their negatives (face centers)
neg = -verts
ax.scatter(neg[:,0], neg[:,1], neg[:,2], color='orange', s=50, label='face centers (negatives)')

# Plot 2-fold axes points (± for each axis)
pts2 = np.vstack([axes2, -axes2])
ax.scatter(pts2[:,0], pts2[:,1], pts2[:,2], color='green', s=60, label='2-fold axes points (6)')
...
plt.tight_layout()
plt.savefig('orbits_tetrahedron.png', dpi=200)
print('Saved orbits_tetrahedron.png')
