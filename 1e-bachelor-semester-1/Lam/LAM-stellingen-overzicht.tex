...
\begin{myenum}
\item Voor alle $A\in M_n(K)$ is $A\,\mathrm{adj}(A)=\mathrm{adj}(A)A=(\det A)I_n$.
\item Voor alle $A\in \mathsf{GL}_n(K)$ is $A^{-1}=(\det A)^{-1}\mathrm{adj}(A)$.
\end{myenum}
\end{res}

\begin{res}{Gevolg}{5.5.5}
Zij $A\in M_{m,n}(K)$, en $k\le\min\{m,n\}$. Dan is $\mathrm{rk}(A)\ge k$ \(\iff\) er een $k\times k$-minor van $A$ is die verschillend is van $0$.
\end{res}
...
Zij $V$ een eindig-dimensionale vectorruimte over een veld $K$, en zij $f\in\mathrm{End}_K(V)$ een lineaire operator, met matrixvoorstelling $A$ (ten opzichte van een willekeurige basis voor $V$). Dan is
\[
\chi_f(x):=\det(xI_n-A)\in K[x]
\]
een monische veelterm van graad $n$ in de variabele $x$, en deze veelterm hangt niet af van de gekozen matrixvoorstelling $A$ van $f$.
\end{res}
...
Een matrix $A\in M_n(K)$ is diagonaliseerbaar \(\iff\) er een matrix $P\in \mathsf{GL}_n(K)$ bestaat zodat $P^{-1}AP$ een diagonaalmatrix is.
\end{res}
...
\]
Voor iedere $i$ is $\dim(\ker(\lambda_i\vec 1_V-f))\le n_i$. Anders gezegd, de meetkundige multipliciteit van een eigenwaarde is steeds kleiner dan of gelijk aan de algebra\"ische multipliciteit van deze eigenwaarde.
\end{res}
...
\chi_f(x)=\prod_{i=1}^t (x-\lambda_i)^{n_i}\quad\text{met } \lambda_1,\dots,\lambda_t\in K.
\]
Zij $V_i=\ker(\lambda_i\vec 1_V-f)$ de eigenruimte behorende bij de eigenwaarde $\lambda_i$, voor elke $i$. Dan is $f$ diagonaliseerbaar \(\iff\) voor elke eigenwaarde $\lambda_i$ geldt dat $\dim(V_i)=n_i$, of met andere woorden, \(\iff\) voor iedere eigenwaarde de algebra\"ische multipliciteit gelijk is aan de meetkundige multipliciteit.
\end{res}
...
\]
en de gelijkheid geldt dan en slechts dan als $v$ en $w$ lineair afhankelijk zijn.
\end{res}

\begin{res}{Gevolg}{8.1.8}
\textup{(Driehoeksongelijkheid.)} Zij $(V,\langle\cdot,\cdot\rangle)$ een inproduct-ruimte over $\mathbb R$ of $\mathbb C$. Dan geldt voor alle $v,w\in V$ dat
\[
\|v+w\|\le\|v\|+\|w\|.
\]
\end{res}
...
Zij $K=\mathbb R$ of $\mathbb C$, en zij $(V,\langle\cdot,\cdot\rangle)$ een eindig-dimensionale inproduct-ruimte over $K$. Voor elke deelruimte $W\le V$ is het orthogonaal complement $W^\perp$ in $V$ een complementaire ruimte, i.e.\ $W\oplus W^\perp=V$. In het bijzonder is $\dim(W^\perp)=\dim V-\dim W$.
\end{res}
...
\[
\mathrm{proj}_W(v)=\sum_{j=1}^k \langle v,w_j\rangle w_j.
\]
\end{myenum}
\end{res}

\begin{res}{Stelling}{8.2.10}
Zij $K=\mathbb R$ of $\mathbb C$, en zij $(V,\langle\cdot,\cdot\rangle)$ een eindig-dimensionale inproduct-ruimte over $K$. Beschouw een deelruimte $W\le V$ en $v\in V$. Dan geldt voor elke $w\in W\setminus\{\mathrm{proj}_W(v)\}$ dat $\mathrm{dist}(v,w)>\mathrm{dist}(v,\mathrm{proj}_W(v))$.
\end{res}

\begin{res}{Stelling}{8.3.1}
\textup{(Gram--Schmidt orthonormalisatieproces.)} Zij $K=\mathbb R$ of $\mathbb C$, en zij $(V,\langle\cdot,\cdot\rangle)$ een eindig-dimensionale inproduct-ruimte over $K$. Zij $W$ een deelruimte van $V$ met basis $\{v_1,\dots,v_d\}$. We defini\"eren nu, op recursieve wijze,
\[
b_1=v_1,\quad b_2=v_2-\mathrm{proj}_{b_1}(v_2),\quad b_3=v_3-\mathrm{proj}_{b_1}(v_3)-\mathrm{proj}_{b_2}(v_3),
\]
\[
b_4=v_4-\mathrm{proj}_{b_1}(v_4)-\mathrm{proj}_{b_2}(v_4)-\mathrm{proj}_{b_3}(v_4),\quad \dots,\quad b_d=v_d-\sum_{i=1}^{d-1}\mathrm{proj}_{b_i}(v_d).
\]
...
Beschouw een inproduct-ruimte $(V,\langle\cdot,\cdot\rangle)$ over $K=\mathbb R$ of $\mathbb C$. Zij $\mathcal B=\{b_1,\dots,b_n\}$ en $\mathcal B'=\{b_1',\dots,b_n'\}$ twee orthonormale basissen van $V$, en zij $Q$ de transitiematrix van $\mathcal B$ naar $\mathcal B'$. Dan is $Q^tQ=I_n$.
\end{res}
...
\item De operator $f$ is diagonaliseerbaar, en er bestaat steeds een orthonormale basis van eigenvectoren voor $f$.
\end{myenum}
\end{res}
...
Alle eigenwaarden van $A$ zijn re\"eel en er bestaat een orthonormale basis van eigenvectoren voor $A$, dus $A$ is diagonaliseerbaar.
